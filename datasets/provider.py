# -*- coding: utf-8 -*-
# Pure PyTorch versions of the rotation utilities — no NumPy, no TensorFlow.
# - All math is done with torch / math.
# - "graph" and other TF-specific arguments are removed.
# - Functions accept/return torch.Tensors on the same device/dtype as inputs.
...
def _as_tensor(x, like=None, dtype=torch.float32):
    """Convert x to a torch tensor. If `like` is provided, match its device/dtype."""
    ...
    if like is not None and isinstance(like, torch.Tensor):
        t = t.to(device=like.device, dtype=like.dtype)
    return t

def _scalar_on(v, like):
    """Return a 0-dim tensor scalar on the same device/dtype as `like`."""
    if isinstance(v, torch.Tensor):
        return v.to(device=like.device, dtype=like.dtype)
    return torch.tensor(v, device=like.device, dtype=like.dtype)

def _build_Rx(ax, like):
    ax = _scalar_on(ax, like)
    cx, sx = torch.cos(ax), torch.sin(ax)
    z = torch.tensor(0.0, device=like.device, dtype=like.dtype)
    o = torch.tensor(1.0, device=like.device, dtype=like.dtype)
    return torch.stack([
        torch.stack([o, z, z]),
        torch.stack([z, cx, -sx]),
        torch.stack([z, sx,  cx]),
...
def _build_Ry(ay, like):
    ay = _scalar_on(ay, like)
...
def _build_Rz(az, like):
    az = _scalar_on(az, like)
...
# ----------------------------
# Core rotations (PyTorch)
# ----------------------------

def rotate_tensor_by_angle_xyz(input_data, angle_x=0.0, angle_y=0.0, angle_z=0.0):
    """
    Rotate an Nx3 point cloud tensor by angles (apply X, then Y, then Z).
      input_data: torch.Tensor [N, 3]
      angle_x/y/z: float or 0-dim torch.Tensor (radians)
    Returns: torch.Tensor [N, 3]
    """
    pc = _as_tensor(input_data, dtype=torch.float32)
    if pc.ndim != 2 or pc.shape[-1] != 3:
        raise ValueError("input_data must be a [N,3] tensor")
    # Build and apply rotation matrices on the same device/dtype
    Rx = _build_Rx(angle_x, pc)
    Ry = _build_Ry(angle_y, pc)
    Rz = _build_Rz(angle_z, pc)
    pc = pc @ Rx
    pc = pc @ Ry
    pc = pc @ Rz
    return pc
...
def rotate_tensor_point_cloud(point_cloud):
    """
    Randomly rotate a single Nx3 point cloud around x, y, z.
      point_cloud: torch.Tensor [N,3]
    Returns: torch.Tensor [N,3]
    """
    pc = _as_tensor(point_cloud, dtype=torch.float32)
    rand = torch.rand(3, device=pc.device, dtype=pc.dtype) * (2 * math.pi)
    ax, ay, az = rand[0], rand[1], rand[2]
    return rotate_tensor_by_angle_xyz(pc, ax, ay, az)

# ----------------------------
# Direction sets (labels → rotations)
# ----------------------------

def rotate_tensor_by_label(batch_data, label):
    """
    18-direction scheme (like original): rotate each sample by a rule from its label (0..17).
      batch_data: torch.Tensor [B,N,3]
      label: torch.Tensor/list/array of length B (ints in [0,17])
    Returns: torch.Tensor [B,N,3]
    """
    X = _as_tensor(batch_data, dtype=torch.float32)
    if X.ndim != 3 or X.shape[-1] != 3:
        raise ValueError("batch_data must be [B,N,3]")
    B = X.shape[0]
    lab = _as_tensor(label, dtype=torch.long, like=X).view(-1).cpu().tolist()
    out = []
    for k in range(B):
        l = int(lab[k])
        pc = X[k]
...
def rotate_tensor_by_label_32(batch_data, label):
    """
    32-direction scheme (icosahedron+dodecahedron).
      batch_data: torch.Tensor [B,N,3]
      label: torch.Tensor/list length B (ints in [0,31])
    Returns: torch.Tensor [B,N,3]
    """
    X = _as_tensor(batch_data, dtype=torch.float32)
    B = X.shape[0]
...
def rotate_tensor_by_label_54(batch_data, label):
    """
    54-direction scheme (2 poles; 14 @30°; 24 @60°; 14 equator).
      batch_data: torch.Tensor [B,N,3]
      label: torch.Tensor/list length B (ints in [0,53])
    Returns: torch.Tensor [B,N,3]
    """
    X = _as_tensor(batch_data, dtype=torch.float32)
    B = X.shape[0]
...
# ----------------------------
# Label → angle maps (torch)
# ----------------------------

def map_label_to_angle_18(label):
    """
    Map a batch of labels to (ax, ay, az) angles.
      label: [B]-like int tensor/list
    Returns: list of B tensors, each shape [3] (ax, ay, az)
    """
    lab = _as_tensor(label, dtype=torch.long).view(-1).cpu().tolist()
    angles = []
    for l in lab:
        l = int(l)
        if l == 0:
...
def map_label_to_angle_32(label):
    """
    Map a batch of labels (0..31) to (ax, ay, az) angles.
    Returns: list of B tensors, each shape [3].
    """
    lab = _as_tensor(label, dtype=torch.long).view(-1).cpu().tolist()
    angles = []
    for l in lab:
...
# ----------------------------
# Angle computations (torch)
# ----------------------------

def compute_angle_tensor(pts1, pts2):
    """
    Compute the angle between corresponding 3D vectors (relative to origin).
      pts1, pts2: either list of [3]-tensors OR a single [B,3] tensor each.
    Returns: torch.Tensor [B] (radians in [0, pi])
    """
    def _to_B3(x):
        if isinstance(x, (list, tuple)):
            t = torch.stack([_as_tensor(v, dtype=torch.float32) for v in x], dim=0)
        else:
            t = _as_tensor(x, dtype=torch.float32)
            if t.ndim == 1:
                t = t.unsqueeze(0)
        return t
...
def get_rotated_angle_diff(num_angles, label1, label2):
    """
    Angle between two rotation directions (by label).
      num_angles: int (<=18) or 32
      label1, label2: [B]-like
    Returns: torch.Tensor [B] (radians)
    """
    l1 = _as_tensor(label1, dtype=torch.long).view(-1)
    l2 = _as_tensor(label2, dtype=torch.long).view(-1)
    if num_angles <= 18:
...
# ----------------------------
# Batch rotations from spherical dirs
# ----------------------------

def rotate_tensor_by_batch(batch_data, label):
    """
    Rotate an entire batch using a single sunflower-based direction derived from `label`.
      batch_data: torch.Tensor [B,N,3]
      label: int-like index into sunflower_distri(B)
    Returns: torch.Tensor [B,N,3]
    """
    X = _as_tensor(batch_data, dtype=torch.float32)
    B = X.shape[0]
    pts = sunflower_distri(B, device=X.device, dtype=X.dtype)
    l = int(_as_tensor(label).item())
    pt = pts[l]  # [3]

    # Compute phi = angle between [x,y,0] and [1,0,0]
...
def rotate_point_by_label_n(batch_data, label, n, distri='sunflower'):
    ...
    X = _as_tensor(batch_data, dtype=torch.float32)
    B = X.shape[0]
    lab = _as_tensor(label, dtype=torch.long, like=X).view(-1)
    ...
    # Shuffle directions like original behavior (optional)
    ...
    for k in range(B):
        ...
        theta = compute_angle_tensor(pt.unsqueeze(0), ez.unsqueeze(0))[0]
        pc = rotate_tensor_by_angle_xyz(pc, angle_x=theta, angle_z=phi)
        out.append(pc)
    return torch.stack(out, dim=0)
